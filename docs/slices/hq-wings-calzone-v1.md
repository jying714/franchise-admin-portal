...
**Status**: **Complete** (W0–W7 + W2 franchise sauce pool)  
**Branch**: `feat/menu-modifier-system-rebuild-v1` (merged to `main`)  
...
| Sauce pool | Items **bind** existing ingredients; franchise shared pool is **W2 Done** |
...
| Data / UX | Same as pizza (included, optionalAddOns, sizes, cheeses/sauces sections) |
| Difference | **No left/right portion UI**; Order Details not required for calzone product path |
...
| Layer | Store | Role |
|--------|--------|------|
| Franchise default | `franchises/{id}/config/menu_profile_wings` | `{ sauceIngredientIds: string[], maxFlavorPortions: 2 }` (**W2 Done**) |
...
Mobile read order: item option lists → modifier groups → franchise `config/menu_profile_wings` → empty (Plain only).
...
| **W2** | Franchise `config/menu_profile_wings` read/write + Apply pool | **Done** (HQ widget + mobile fallback + effective ids; July 28) |
...
| **W7** | Smoke acceptance | **Done** (human PASS July 28) |

### Suggested build order (executed)

W0 → W1 → W5/W4 → W3 → W6 → **W7** → **W2** → **M5** of menu-modifier epic.

## Acceptance

**Wings**
- [x] Sizes drive free cups + extra upcharge from item maps  
- [x] Always 2 flavor portions; Plain = no toss  
- [x] Sauces from item bind; toss list = side-cup list  
- [x] One **Dipping sauces** section (counts); no dual empty dips taxonomy  
- [x] Cart/total correct; empty pool does not crash  
- [x] HQ free-cups panel + save projection of bound sauces  
- [x] Mobile sauces list smoke PASS (human)  
- [x] W2 franchise pool doc load/save + Apply to item (HQ)  
- [x] W2 mobile fallback when item lists empty (`sauceIdsOverride` + `_effectiveWingSauceIds`)  

**Calzone**
- [x] `menuProfile: calzone` end-to-end acceptance  
- [x] Profile + pizza path helpers (mobile/web)  
- [x] No left/right half UI  
- [x] Cheeses / Sauces parity with pizza  

**Regression**
- [x] Pizza CBR optionalAddOns contract + Order Details structural (template fallback)  
- [x] Standard / salad / dinner usable (Order Details off; optional only when set)  
...
- M5 dual-tree deletion (separate epic gate — complete)  
...
- `web-app/.../wings_franchise_sauce_pool.dart` (W2 HQ)  
...
- `mobile_app/.../main_menu_screen.dart` + `confirmation_screen.dart` (deferred order-experience)  
